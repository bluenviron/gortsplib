	...
	},
	{
		...
		&format.H264{
			...
			SPS: []byte{
				...
				0x03, 0x00, 0x96, 0xa0, 0x03, 0xc0, 0x80, 0x10,
				0xe5, 0x8d, 0xae, 0x49, 0x32, 0x6b, 0x96, 0x70,
				0x08, 0x00, 0x00, 0x03, 0x00, 0x08, 0x00, 0x00,
				0x03, 0x00, 0xc8, 0x40,
			},
			PPS: []byte{0x44, 0x01, 0xe0, 0x76, 0xb0, 0x26, 0x40},
		},
		96,
		"H264/90000",
		map[string]string{
...
func TestUnmarshal(t *testing.T) {
	for _, ca := range casesFormat {
		t.Run(ca.name, func(t *testing.T) {
			...
			dec, err := format.Unmarshal(desc.MediaDescriptions[0], desc.MediaDescriptions[0].MediaName.Formats[0])
			require.NoError(t, err)
...
func FuzzUnmarshal(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, in string) {
		...
		f, err := format.Unmarshal(desc.MediaDescriptions[0], desc.MediaDescriptions[0].MediaName.Formats[0])
		if err != nil {
			return
		}

		// only Generic can return zero ClockRate
		if _, ok := f.(*format.Generic); !ok {
