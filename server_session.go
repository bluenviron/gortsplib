	...
	for {
					...
					res.Header["Session"] = headers.Session{
						...
						Timeout: func() *uint {
							...
							if (ss.state == ServerSessionStatePrePlay ||
								...
								(*ss.setuppedProtocol == TransportUDP ||
									*ss.setuppedProtocol == TransportUDPMulticast) {
		...
		case sc := <-ss.chRemoveConn:
			...
			if ((ss.state != ServerSessionStateRecord &&
				ss.state != ServerSessionStatePlay) ||
				*ss.setuppedProtocol == TransportTCP) &&
				len(ss.conns) == 0 {
...
func (ss *ServerSession) handleRequestInner(sc *ServerConn, req *base.Request) (*base.Response, error) {
	...
	case base.Setup:
		...
		if ss.setuppedProtocol != nil && (*ss.setuppedProtocol != protocol || ss.setuppedSecure != inTH.Secure) {
			return &base.Response{
				StatusCode: base.StatusBadRequest,
