...
func TestServerPlay(t *testing.T) {
	for _, ca := range []struct {
		...
		t.Run(ca.scheme+"_"+ca.transport+"_"+ca.secure, func(t *testing.T) {
			...
			switch ca.transport {
			case "udp":
				inTH.Delivery = new(headers.TransportDeliveryUnicast)
				inTH.Protocol = headers.TransportProtocolUDP
				inTH.ClientPorts = &[2]int{35466, 35467}

			case "multicast":
				inTH.Delivery = new(headers.TransportDeliveryMulticast)
				inTH.Protocol = headers.TransportProtocolUDP
...
func TestServerPlayPlayPausePausePlay(t *testing.T) {
	for _, protocol := range []string{
		...
		for _, mode := range []string{"stream", "direct"} {
			t.Run(protocol+"_"+mode, func(t *testing.T) {
				var stream *ServerStream
				writerStarted := false
				writerDone := make(chan struct{})
				writerTerminate := make(chan struct{})
