			...
			s := &Server{
				Handler: &testServerHandler{
					onAnnounce: func(ctx *ServerHandlerOnAnnounceCtx) (*base.Response, error) {
						// TODO: choose available methods

						/* var methods []headers.AuthMethod
						if method == "basic" {
							methods = []headers.AuthMethod{headers.AuthMethodBasic}
						} else if method == "digest" {
							methods = []headers.AuthMethod{headers.AuthMethodDigest}
						} */

						res, err := ctx.Conn.VerifyCredentials(ctx.Request, "myuser", "mypass")
						if err != nil {
							return res, err
						}
...
func TestServerAuthFail(t *testing.T) {
	s := &Server{
		Handler: &testServerHandler{
			...
			onAnnounce: func(ctx *ServerHandlerOnAnnounceCtx) (*base.Response, error) {
				res, err := ctx.Conn.VerifyCredentials(ctx.Request, "myuser2", "mypass2")
				if err != nil {
					return res, err
				}
